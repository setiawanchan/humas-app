...
// Helper Format Tanggal Indonesia (contoh: 14 Agustus 2026)
...
export default function ArsipPage() {
  ...
  // Daftar opsi Tahun untuk filter
  const availableYears = useMemo(() => {
    const yearsSet = new Set<string>();
    yearsSet.add(currentYearStr);
    items.forEach((item) => {
      if (item.tanggal) {
        const y = item.tanggal.split("-")[0];
        if (y) yearsSet.add(y);
      }
    });
  ...
  const filteredAndSortedItems = useMemo(() => {
    let result = items.filter((item) => {
      ...
      if (searchQuery.trim() !== "") {
        ...
        const matchesTitle = item.judul.toLowerCase().includes(q);
        const matchesDesc = item.deskripsi.toLowerCase().includes(q);
      ...
      // 3. Filter Tahun
      if (selectedYear !== "all" && item.tanggal) {
        const itemYear = item.tanggal.split("-")[0];
    ...
    return result;
  }, [items, searchQuery, selectedCategory, selectedYear, sortColumn, sortDirection]);
  ...
  // Buka Form Edit
  const handleOpenEditForm = (item: Arsip, e?: React.MouseEvent) => {
  ...
  // Trigger Hapus
  const handleOpenDeleteConfirm = (item: Arsip, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Penyimpanan dokumen administratif resmi (SK, Logo & Aset Visual, Template Laporan) BPS Kabupaten Lebak
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-3 items-center">
          ...
          <div className="md:col-span-3">
            ...
            >
              ...
              <option value="logo_aset">🎨 Logo & Aset Visual</option>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 px-1">
        <div>
          Menampilkan <span className="font-bold text-brand-600 dark:text-brand-400">{filteredAndSortedItems.length}</span> dari {items.length} Arsip
        </div>
        <div>
          * Klik item untuk melihat detail atau buka file Drive
        </div>
      </div>

      {/* Data Table Tampilan Arsip */}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-700/50 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider select-none">
                ...
                <th
                  ...
                  className="py-3.5 px-4 w-40 cursor-pointer hover:bg-gray-100/60 dark:hover:bg-gray-700/80 transition"
                >
                ...
                {/* 3. Kolom Judul & Deskripsi */}
                <th
                  onClick={() => handleSort("judul")}
                  className="py-3.5 px-4 cursor-pointer hover:bg-gray-100/60 dark:hover:bg-gray-700/80 transition"
                >
                  <div className="flex items-center gap-1">
                    <span>Judul Dokumen & Deskripsi</span>
                    {renderSortIndicator("judul")}
                  </div>
                </th>

                {/* 4. Kolom File Drive */}
                <th className="py-3.5 px-4 w-44 text-center">Link File</th>

                {/* 5. Kolom Aksi (Khusus Pengelola) */}
                {canManage && (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700/60 text-sm">
              {filteredAndSortedItems.length === 0 ? (
                <tr>
                  <td colSpan={canManage ? 6 : 5} className="p-8 text-center text-gray-500 dark:text-gray-400 text-xs">
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4">
                        <div className="font-semibold text-gray-900 dark:text-white hover:text-brand-600 dark:hover:text-brand-400 transition">
                          {item.judul}
                        </div>
      ...
      {/* ===================== MODAL DETAIL ARSIP ===================== */}
      {detailItem && (
        <Modal isOpen={!!detailItem} onClose={() => setDetailItem(null)} showCloseButton={true} className="max-w-lg p-6">
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${getCategoryBadge(detailItem.kategori).className}`}>
                {getCategoryBadge(detailItem.kategori).label}
              </span>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Tanggal Terbit/Upload: <strong className="text-gray-700 dark:text-gray-300">{formatIndonesianDate(detailItem.tanggal)}</strong>
              </p>
            </div>

            <div className="p-3.5 rounded-xl bg-gray-50 dark:bg-gray-800 text-xs text-gray-700 dark:text-gray-300 leading-relaxed border border-gray-100 dark:border-gray-700">
              {detailItem.deskripsi || "Tidak ada deskripsi tambahan."}
            </div>
            ...
            <div className="pt-4 flex items-center justify-between border-t border-gray-100 dark:border-gray-700">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white text-xs font-semibold shadow transition"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                </svg>
      ...
      {isFormOpen && (
        <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} showCloseButton={false} className="max-w-lg p-6">
          <div className="space-y-4">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
              {editingItem ? "Edit Dokumen Arsip" : "Tambah Dokumen Arsip Baru"}
            </h2>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4 text-sm">
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: SK Tim Kehumasan BPS Lebak 2026"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, kategori: e.target.value as Arsip["kategori"] })}
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">
                  Tanggal Dokumen (Format: DD-MM-YYYY) *
                </label>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Ringkasan isi dokumen atau panduan penggunaan..."
                  className="w-full p-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
                />
              </div>
        ...
        <Modal isOpen={!!deletingItem} onClose={() => setDeletingItem(null)} showCloseButton={false} className="max-w-md p-6">
          <div className="text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Apakah Anda yakin ingin menghapus dokumen arsip <strong className="text-gray-900 dark:text-white">"{deletingItem.judul}"</strong>? Tindakan ini tidak dapat dibatalkan.
              </p>
            </div>
